...
export async function uploadReceipt(file) {
  ...
  // Get a unique ID for the new receipt
  const newId = `rec-${receipts.length + 1}`;
  
  // Create a new receipt with sample extracted data
  const newReceipt = {
    id: newId,
    ...
    total: 23.45,
    items: [
      { id: `item-${Date.now()}-1`, description: "Cappuccino", amount: 5.99 },
      { id: `item-${Date.now()}-2`, description: "Croissant", amount: 3.49 },
      { id: `item-${Date.now()}-3`, description: "Sandwich", amount: 8.99 },
      { id: `item-${Date.now()}-4`, description: "Orange Juice", amount: 4.98 }
    ],
    allocations: {},
  };
  
  // In a real app, we would process the file to extract data
  // For now, we're using predefined mock data
